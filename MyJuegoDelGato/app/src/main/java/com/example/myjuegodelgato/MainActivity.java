...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
   ...
   // private AdView mAdView;
    ...
    private void clickButton(Button button, int number) {
        ...
        checkGame();
        numbers.add(number);
        if(isCorner(number)) corners.add(number);
    ...
    private int sendBestCell(){
        int co = adyacentesX();
        if (co != 0)
            return co;
        int cell = adyacentesO();
        if (cell != 0)
            return cell;
        if (!box5Player1 && available(5))
            return 5;
         else
            return getCorner();
    ...
    private int getCorner() {
        int i = 0 ;
        int counter = 0;
        if(corners.size()==4)return getNumber();
        boolean flag = false;
        while (!flag && counter++<10) {
            i = ((int) (Math.random() * 9) + 1);
            if (available(i) && isCorner(i)) {
                flag = true;
            }
        }
        if(counter>=10) return getNumber();
        return i;
    }

    private int adyacentesO() {
        if( box1Player1 && box2Player1)
            if(available(3))return 3;
        if( box4Player1 && box5Player1)
            if(available(6))return 6;
        if( box7Player1 && box8Player1)
            if(available(9))return 9;
        if( box1Player1 && box4Player1)
            if(available(7))return 7;
        if( box2Player1 && box5Player1)
            if(available(8))return 8;
        if( box3Player1 && box6Player1)
            if(available(9))return 9;
        if( box1Player1 && box5Player1)
            if(available(9))return 9;
        if( box3Player1 && box5Player1)
            if(available(7))return 7;

        if( box3Player1 && box2Player1)
            if(available(1))return 1;
        if( box6Player1 && box5Player1)
            if(available(4))return 4;
        if( box9Player1 && box8Player1)
            if(available(7))return 7;
        if( box7Player1 && box4Player1)
            if(available(1))return 1;
        if( box8Player1 && box5Player1)
            if(available(2))return 2;
        if( box9Player1 && box6Player1)
            if(available(3))return 3;
        if( box9Player1 && box5Player1)
            if(available(1))return 1;
        if( box7Player1 && box5Player1)
            if(available(3))return 3;

        if(box1Player1 && box3Player1)
            if(available(2))return 2;
        if(box4Player1 && box6Player1)
            if(available(5))return 5;
        if(box7Player1 && box9Player1)
            if(available(8))return 8;
        if(box1Player1 && box7Player1)
            if(available(4))return 4;
        if(box2Player1 && box8Player1)
            if(available(5))return 5;
        if(box3Player1 && box9Player1)
            if(available(6))return 6;

        return 0;
    }

    private int adyacentesX() {
        if( box1Player2 && box2Player2)
            if(available(3))return 3;
        if( box4Player2 && box5Player2)
            if(available(6))return 6;
        if( box7Player2 && box8Player2)
            if(available(9))return 9;
        if( box1Player2 && box4Player2)
            if(available(7))return 7;
        if( box2Player2 && box5Player2)
            if(available(8))return 8;
        if( box3Player2 && box6Player2)
            if(available(9))return 9;
        if( box1Player2 && box5Player2)
            if(available(9))return 9;
        if( box3Player2 && box5Player2)
            if(available(7))return 7;

        if( box3Player2 && box2Player2)
            if(available(1))return 1;
        if( box6Player2 && box5Player2)
            if(available(4))return 4;
        if( box9Player2 && box8Player2)
            if(available(7))return 7;
        if( box7Player2 && box4Player2)
            if(available(1))return 1;
        if( box8Player2 && box5Player2)
            if(available(2))return 2;
        if( box9Player2 && box6Player2)
            if(available(3))return 3;
        if( box9Player2 && box5Player2)
            if(available(1))return 1;
        if( box7Player2 && box5Player2)
            if(available(3))return 3;

        if(box1Player2 && box3Player2)
            if(available(2))return 2;
        if(box4Player2 && box6Player2)
            if(available(5))return 5;
        if(box7Player2 && box9Player2)
            if(available(8))return 8;
        if(box1Player2 && box7Player2)
            if(available(4))return 4;
        if(box2Player2 && box8Player2)
            if(available(5))return 5;
        if(box3Player2 && box9Player2)
            if(available(6))return 6;
    ...
    private void changeTurn() {
        if (TURN == PLAYER_ONE) {
            TURN = PLAYER_TWO;
            if(computerPlays)
                txtTurn.setText(String.format(Locale.getDefault(),
                        "%s %s",getString(R.string.turnsCompu) , s));
            else
